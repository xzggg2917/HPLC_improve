...
### 归一化公式
```
Score_sub = min(100, (Σ(mᵢ × fₛᵤᵦ) / baseline_mass) × 100)
```

**参数说明**:
- `mᵢ`: 第i个试剂的质量 (克)
- `fₛᵤᵦ`: 该试剂的小因子值 (0-1范围)
- `baseline_mass`: 归一化基准质量，根据色谱类型确定
- `100`: 上限，防止分数超过100

### 色谱类型基准质量

| 色谱类型 | baseline_mass | 说明 |
|---------|--------------|------|
| UPLC | 4.0 g | 超高效液相色谱 |
| HPLC_UV | 45.0 g | 标准液相色谱（UV检测器） |
| HPLC_MS | 10.0 g | 液相色谱-质谱联用 |
| PrepHPLC | 250.0 g | 制备型液相色谱 |
| SFC | 4.0 g | 超临界流体色谱 |
...
**Step 2**: 归一化到0-100分制
```python
score = (weighted_sum / baseline_mass) × 100
...
权重（Balanced）:
  wₛ=0.15, wₕ=0.15, wₑ=0.15, wₚ=0.35, wᵣ=0.10, wᵤ=0.10

计算:
Score₁ = 53.78×0.15 + 29.17×0.15 + 45.67×0.15 +
         28.57×0.35 + 0.00×0.10 + 100.00×0.10
       = 8.07 + 4.38 + 6.85 + 10.00 + 0.00 + 10.00
       = 39.30分
```
...
#### 公式
```
Score₂ = S×wₛ + H×wₕ + E×wₑ + R×wᵣ + D×wᵤ
```

**注意**: 前处理阶段**没有P因子**（能耗只在仪器分析阶段计算）
...
权重（Balanced）:
  wₛ=0.20, wₕ=0.20, wₑ=0.20, wᵣ=0.20, wᵤ=0.20

计算:
Score₂ = 45.23×0.20 + 32.45×0.20 + 56.78×0.20 +
         0.00×0.20 + 100.00×0.20
       = 9.05 + 6.49 + 11.36 + 0.00 + 20.00
       = 46.90分
```
